...
function create() {
    ...
    slotMachineBg = game.add.image(155, 12, 'Slotmachine');
    ...
    for(let reel = 0; reel < 4; reel++) {
        slotmachine[reel] = new Object();
 
        for(let slot = 0; slot < 3; slot++) {
            let image = slotSelection();
            if(slot != 0) {
                // Aligns to the previous slot in the reel.
                image.alignTo(slotmachine[reel][slot - 1], Phaser.BOTTOM_CENTER);
            }
    ...
    // Scales images down.
    slotMachineBg.scale.setTo(0.62, 0.62);
...
function slotSelection(type) {
    ...
    switch(type) {
        case 0:
            image = game.add.image(0, 175, 'Slots_Bar');
            break;
        case 1:
            image = game.add.image(0, 175, 'Slots_Lemon');
            break;
        case 2:
            image = game.add.image(0, 175, 'Slots_Watermelon');
            break;
        case 3:
            image = game.add.image(0, 175, 'Slots_Ten');
            break;
        case 4:
            image = game.add.image(0, 175, 'Slots_Crown');
            break;
        case 5:
            image = game.add.image(0, 175, 'Slots_Seven');
            break;
        case 6:
            image = game.add.image(0, 175, 'Slots_Diamond');
            break;
    }
